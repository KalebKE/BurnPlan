...
class DocContextTests(unittest.TestCase):
    ...
    def test_generated_docs_are_excluded_from_context(self):
        # A promoted BurnPlan doc must not feed back into synthesis as if it
        # were human-written; that would be a self-reference loop.
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "docs").mkdir()
            (root / "docs" / "design.md").write_text(
                "# Design\n\n<!-- GENERATED by burnplan. Review before treating as truth. -->\n\nStuff.\n",
                encoding="utf-8",
            )
            base_map = _base_map([{"path": "docs/design.md", "kind": "design", "title": "Design"}])

...
def build_project_proposals(
    ...
    outputs.update({f"proposals/{path}": content for path, content in codex.items()})
...
def _render_generic_agent(
    ...
    lines.extend(
        [
            ...
            "## Context",
            "",
            "The orchestrator normally resolves a task-scoped route pack and passes it in the spawn prompt:",
            "",
            "```sh",
    ...
    tools = subagent.get("tools") or []
    if tools:
        lines.append(f"- Tools: {', '.join(f'`{tool}`' for tool in tools)}")
    effort = subagent.get("effort")
    if effort:
        lines.append(f"- Suggested effort: `{effort}` (orchestrator-facing; spend high effort where decisions are made)")
    for instruction in subagent.get("instructions", []) or []:
        lines.append(f"- {instruction}")
    lines.append("- Return a distilled summary to the parent thread; keep raw logs, transcripts, and long command output out of it.")
    lines.append("")
    return "\n".join(lines)
...
AGENTS_MD_BEGIN = "<!-- burnplan:begin -->"
AGENTS_MD_END = "<!-- burnplan:end -->"


def render_codex_wiring_proposal() -> Dict[str, str]:
    block = "\n".join(
        [
            AGENTS_MD_BEGIN,
            "## BurnPlan ratchet",
            "",
            "This repo carries a BurnPlan context ratchet. Read `.burnplan/agent-prompts.md`",
            "first; it holds the read-first list and the pre-work checklists.",
            "",
            "For a task-scoped context slice (where to start, likely edits, relevant tests,",
            "blast radius), have the orchestrator run:",
            "",
            "    burnplan assign --team <team> --behavior <behavior> --task-file <task> --format json",
            "",
            "Subagent role specs live in `docs/agents/`. Keep heavy exploration and log",
            "reading inside a subagent and return a distilled summary to the main thread.",
            AGENTS_MD_END,
        ]
    )
    readme = "\n".join(
        [
            ...
            "<!-- GENERATED by burnplan. Review before promoting. -->",
            "",
            "`AGENTS-block.md` is a small marker-fenced block for the repository's `AGENTS.md`,",
            "the file Codex loads automatically. It is deliberately tiny: pointers into the",
            "ratchet, the route-pack command, and one boundary about distilled summaries.",
            "Architectural rules stay in the files it points at, loaded as needed.",
            "",
            "`burnplan promote agents` installs it:",
            "",
            "- If `AGENTS.md` does not exist, it is created containing only the block.",
            "- If it exists and already has a burnplan block, the content between the markers",
            "  is replaced in place, so re-promotion updates rather than duplicates.",
            "- If it exists without a block, the block is appended. Nothing outside the",
            "  markers is ever touched.",
            "",
        ]
    )
    return {
        "agents/codex/AGENTS-block.md": block + "\n",
        "agents/codex/README.md": readme,
    }


def _promote_codex_wiring(proposal_root: Path, repo_root: Path) -> List[Path]:
    source = proposal_root / "agents" / "codex" / "AGENTS-block.md"
    ...
    if AGENTS_MD_BEGIN in existing and AGENTS_MD_END in existing:
        head, _sep, rest = existing.partition(AGENTS_MD_BEGIN)
        _old, _sep2, tail = rest.partition(AGENTS_MD_END)
